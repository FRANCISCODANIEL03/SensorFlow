...
function App() {
  ...
  const cargarSensores = async () => {
    ...
    try {
      ...
      const respuesta = await fetch(`${import.meta.env.VITE_URL_API || 'http://localhost:3001'}/api/sensores`);
      if (!respuesta.ok) {
  ...
  const eliminarSensor = async (id) => {
    ...
    try {
      await fetch(`${import.meta.env.VITE_URL_API || 'http://localhost:3001'}/api/sensores/${id}`, {
        method: 'DELETE'
      });
  ...
  const [filtroTipo, setFiltroTipo] = useState('todos'); const sensoresFiltrados = filtroTipo === 'todos'
    ? sensores
    ...
    <div className="contenedor">
      ...
      <form onSubmit={agregarSensor} className="formulario">
        ...
        <input
          ...
          type="number"
          placeholder="Valor"
          value={formulario.valor} onChange={manejarCambio}
          required
